...
NOTE: If a value submitted to the 'add2' function is a float, it must be done
so in quotes (i.e. as a string).
'''

# If 'value' is not an integer, convert it to a float
# and failing that, a string.
def conv(value):
    try:
        return int(value)
    except ValueError:
